const { getSingleUser, getAllUsers, getSinglegiftCardTransaction, getAllGiftCardTransactions, getAllWalletTransactions, createAdminAccount, loginAdmin, decideGiftCardTransanction, decideWalletTransanction, allPendingGiftcardTransactions, allPendingWalletTransactions, allCompletedWalletTransactions, allCompletedGiftcardTransactions, allUsersReferrals } = require("../services/admin.service");
...
try {
  ...
  return res
      .status(201)
      .json(sendSuccessMessage(transaction.status, 201));
} catch (error) {
  return res
      .status(error.status)
      .json(sendErrorMessage(error.message, error.status ?? 500));
